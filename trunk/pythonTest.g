...
caseElementsStm
  : ( test COLON NEWLINE codeBlock )+ 'default' COLON NEWLINE codeBlock
  ;
...
repeatUntilStm
  : 'repeat' NEWLINE codeBlock 'until' test
  ;
	...
	{ getCharPositionInLine() == 0 }?=> 
		...
		)+
		{
			...
			if ( nSpaces > lastIndentation ) {
				...
				Token t = new ClassicToken(INDENT, new String(" ")); t.setLine($line);
				emit( t );
